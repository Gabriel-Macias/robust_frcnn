...
def calc_iou(R, img_data, C, class_mapping, mode = "source", eps = 0.05):
	...
	for ix in range(R.shape[0]):
		...
		if best_iou < C.classifier_min_overlap:
			...
			if C.classifier_min_overlap <= best_iou < C.classifier_max_overlap:
				...
				if mode != "source":
					soft_labels = bboxes[best_bbox]['probas']
		...
		if mode == "source":
			...
			class_label = soft_labels.tolist()
		else:
			class_label = len(class_mapping) * [eps / (len(class_mapping) - 1)]
			class_label[-1] = 1 - eps
