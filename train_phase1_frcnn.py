...
parser.add_option("--network", dest="network", help="Base network to use. Supports all 7 options in keras_frcnn.", default='vgg')
...
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='phase1_frcnn.hdf5')
parser.add_option("--input_weight_path", dest="input_weight_path", help="Input path for base net weights. If not specified, will try to load default weights provided by keras.", default=None)
parser.add_option("--rpn", dest="rpn_weight_path", help="Input path for pre-trained rpn weights.", default=None)
parser.add_option("--opt", dest="optimizers", help="Set the optimizer to use.", default="SGD")
parser.add_option("--elen", dest="epoch_length", help="Nb steps per epoch. def=1000", default=1000)
parser.add_option("--load", dest="load", help="Path to model weights for fine tuning", default=None)
parser.add_option("--cat", dest="cat", help="Categroy to train on. Default train on all cats.", default=None)
parser.add_option("--lr", dest="lr", help="Learning rate (def 1e-3)", type=float, default=1e-3)
...
if not os.path.isdir("models"):
	os.mkdir("models")
if not os.path.isdir("models/phase1"):
	os.mkdir("models/phase1")
C.model_path = os.path.join("models/phase1", options.network + "_" + options.output_weight_path)
...
if options.optimizers == "SGD":
	...
	optimizer = Adam(lr = options.lr, clipnorm = 1e-5)
	optimizer_classifier = Adam(lr = options.lr, clipnorm = 1e-5)
	...
	while True:
		try:
			...
			progbar.update(iter_num, [('rpn_cls', np.mean(losses[:iter_num, 0])), ('rpn_regr', np.mean(losses[:iter_num, 1])),
									  ...
									  ("avg nb of objects", len(selected_pos_samples))])
